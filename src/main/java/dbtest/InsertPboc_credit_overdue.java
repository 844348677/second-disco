...
public class InsertPboc_credit_overdue {

	public static void main(String[] args) throws Exception{
		String csvRoot = "data/data_dev/";
		String tableName = "Pboc_credit_overdue.csv";
...
/*		ReadCsvLine rcl = new ReadCsvLine();
		...
		for(int i=0;i<list.size();i++){
			Pboc_credit_overdue idi = new Pboc_credit_overdue();
			idi.setId((long)(i+1));
			...
			for(int j=0;j<a.length;j++){
				idi.setReportId(a[0]);
				idi.setPboc_credit_overdue_OverdueMonthNum(a[1]);
				idi.setPboc_credit_overdue_OverdueMonth(a[2]);
				idi.setPboc_credit_overdue_credit_num(a[3]);
				idi.setPboc_credit_overdue_OverdueAmount(a[4]);
