...
import com.hdd.dbtest.domain.Resideinfo;

public class InsertResideinfo {

	public static void main(String[] args) throws Exception{
		String csvRoot = "data/data_dev/";
		String tableName = "Resideinfo.csv";
...
/*		ReadCsvLine rcl = new ReadCsvLine();
		...
		for(int i=0;i<list.size();i++){
			...
			for(int j=0;j<a.length;j++){
				idi.setReportId(a[0]);
				idi.setNO(a[1]);
				idi.setResideinfo_ResideStatus(a[2]);
				
				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd"); 
				String strDate = a[3];
				if(strDate != null && !"".equals(strDate)){
					Date date = sdf.parse(strDate);
					idi.setResideinfo_InfoUpdateDate(date);
				}
