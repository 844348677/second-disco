...
public class InsertOccupationinfo {
	
	public static void main(String[] args) throws Exception{
		String csvRoot = "data/data_dev/";
		String tableName = "Occupationinfo.csv";
...
/*		ReadCsvLine rcl = new ReadCsvLine();
		...
		for(int i=0;i<list.size();i++){
			...
			for(int j=0;j<a.length;j++){
				idi.setReportId(a[0]);
				idi.setOccupation(a[1]);
				idi.setCompanyType(a[2]);
				idi.setPosition(a[3]);
				idi.setPostTitle(a[4]);
				...
				idi.setOccupationinfo_NoEmploy(a[6]);
				
				String str = a[7];
				if(str != "0" && !"0".equals(str)){	
					idi.setOccupationinfo_EmployedYear(a[7]);
				}
