package domain;

public class NewData {

	private long id;
	private String zaa;
	private String zab;
	private String zac;
	private String zad;
	private String zae;
	private String zaf;
	private String zag;
	private String zah;
	private String zai;
	private String zaj;
	private String zak;
	private String zal;
	private String zam;
	private String zan;
	private String zao;
	private String zap;
	private String zaq;
	private String zar;
	private String zas;
	private String zat;
	private String zau;
	private String zav;
	private String zaw;
	private String zax;
	private String zay;
	private String zaz;
	private String zba;
	private String zbb;
	private String zbc;
	private String zbd;
	private String zbe;
	private String zbf;
	private String zbg;
	private String zbh;
	private String zbi;
	private String zbj;
	private String zbk;
	private String zbl;
	private String zbm;
	private String zbn;
	private String zbo;
	private String zbp;
	private String zbq;
	private String zbr;
	private String zbs;
	private String zbt;
	private String zbu;
	private String zbv;
	private String zbw;
	private String zbx;
	private String zby;
	private String zbz;
	private String zca;
	private String zcb;
	private String zcc;
	private String zcd;
	private String zce;
	private String zcf;
	private String zcg;
	private String zch;
	private String zci;
	private String zcj;
	private String zck;
	private String zcl;
	private String zcm;
	private String zcn;
	private String zco;
	private String zcp;
	private String zcq;
	private String zcr;
	private String zcs;
	private String zct;
	private String zcu;
	private String zcv;
	private String zcw;
	private String zcx;
	private String zcy;
	private String zcz;
	private String zda;
	private String zdb;
	private String zdc;
	private String zdd;
	private String zde;
	private String zdf;
	private String zdg;
	private String zdh;
	private String zdi;
	private String zdj;
	private String zdk;
	private String zdl;
	private String zdm;
	private String zdn;
	private String zdo;
	private String zdp;
	private String zdq;
	private String zdr;
	private String zds;
	private String zdt;
	private String zdu;
	private String zdv;
	private String zdw;
	private String zdx;
	private String zdy;
	private String zdz;
	private String zea;
	private String zeb;
	private String zec;
	private String zed;
	private String zee;
	private String zef;
	private String zeg;
	private String zeh;
	private String zei;
	private String zej;
	private String zek;
	private String zel;
	private String zem;
	private String zen;
	private String zeo;
	public long getId() {
		return id;
	}
	public void setId(long id) {
		this.id = id;
	}
	public String getZaa() {
		return zaa;
	}
	public void setZaa(String zaa) {
		this.zaa = zaa;
	}
	public String getZab() {
		return zab;
	}
	public void setZab(String zab) {
		this.zab = zab;
	}
	public String getZac() {
		return zac;
	}
	public void setZac(String zac) {
		this.zac = zac;
	}
	public String getZad() {
		return zad;
	}
	public void setZad(String zad) {
		this.zad = zad;
	}
	public String getZae() {
		return zae;
	}
	public void setZae(String zae) {
		this.zae = zae;
	}
	public String getZaf() {
		return zaf;
	}
	public void setZaf(String zaf) {
		this.zaf = zaf;
	}
	public String getZag() {
		return zag;
	}
	public void setZag(String zag) {
		this.zag = zag;
	}
	public String getZah() {
		return zah;
	}
	public void setZah(String zah) {
		this.zah = zah;
	}
	public String getZai() {
		return zai;
	}
	public void setZai(String zai) {
		this.zai = zai;
	}
	public String getZaj() {
		return zaj;
	}
	public void setZaj(String zaj) {
		this.zaj = zaj;
	}
	public String getZak() {
		return zak;
	}
	public void setZak(String zak) {
		this.zak = zak;
	}
	public String getZal() {
		return zal;
	}
	public void setZal(String zal) {
		this.zal = zal;
	}
	public String getZam() {
		return zam;
	}
	public void setZam(String zam) {
		this.zam = zam;
	}
	public String getZan() {
		return zan;
	}
	public void setZan(String zan) {
		this.zan = zan;
	}
	public String getZao() {
		return zao;
	}
	public void setZao(String zao) {
		this.zao = zao;
	}
	public String getZap() {
		return zap;
	}
	public void setZap(String zap) {
		this.zap = zap;
	}
	public String getZaq() {
		return zaq;
	}
	public void setZaq(String zaq) {
		this.zaq = zaq;
	}
	public String getZar() {
		return zar;
	}
	public void setZar(String zar) {
		this.zar = zar;
	}
	public String getZas() {
		return zas;
	}
	public void setZas(String zas) {
		this.zas = zas;
	}
	public String getZat() {
		return zat;
	}
	public void setZat(String zat) {
		this.zat = zat;
	}
	public String getZau() {
		return zau;
	}
	public void setZau(String zau) {
		this.zau = zau;
	}
	public String getZav() {
		return zav;
	}
	public void setZav(String zav) {
		this.zav = zav;
	}
	public String getZaw() {
		return zaw;
	}
	public void setZaw(String zaw) {
		this.zaw = zaw;
	}
	public String getZax() {
		return zax;
	}
	public void setZax(String zax) {
		this.zax = zax;
	}
	public String getZay() {
		return zay;
	}
	public void setZay(String zay) {
		this.zay = zay;
	}
	public String getZaz() {
		return zaz;
	}
	public void setZaz(String zaz) {
		this.zaz = zaz;
	}
	public String getZba() {
		return zba;
	}
	public void setZba(String zba) {
		this.zba = zba;
	}
	public String getZbb() {
		return zbb;
	}
	public void setZbb(String zbb) {
		this.zbb = zbb;
	}
	public String getZbc() {
		return zbc;
	}
	public void setZbc(String zbc) {
		this.zbc = zbc;
	}
	public String getZbd() {
		return zbd;
	}
	public void setZbd(String zbd) {
		this.zbd = zbd;
	}
	public String getZbe() {
		return zbe;
	}
	public void setZbe(String zbe) {
		this.zbe = zbe;
	}
	public String getZbf() {
		return zbf;
	}
	public void setZbf(String zbf) {
		this.zbf = zbf;
	}
	public String getZbg() {
		return zbg;
	}
	public void setZbg(String zbg) {
		this.zbg = zbg;
	}
	public String getZbh() {
		return zbh;
	}
	public void setZbh(String zbh) {
		this.zbh = zbh;
	}
	public String getZbi() {
		return zbi;
	}
	public void setZbi(String zbi) {
		this.zbi = zbi;
	}
	public String getZbj() {
		return zbj;
	}
	public void setZbj(String zbj) {
		this.zbj = zbj;
	}
	public String getZbk() {
		return zbk;
	}
	public void setZbk(String zbk) {
		this.zbk = zbk;
	}
	public String getZbl() {
		return zbl;
	}
	public void setZbl(String zbl) {
		this.zbl = zbl;
	}
	public String getZbm() {
		return zbm;
	}
	public void setZbm(String zbm) {
		this.zbm = zbm;
	}
	public String getZbn() {
		return zbn;
	}
	public void setZbn(String zbn) {
		this.zbn = zbn;
	}
	public String getZbo() {
		return zbo;
	}
	public void setZbo(String zbo) {
		this.zbo = zbo;
	}
	public String getZbp() {
		return zbp;
	}
	public void setZbp(String zbp) {
		this.zbp = zbp;
	}
	public String getZbq() {
		return zbq;
	}
	public void setZbq(String zbq) {
		this.zbq = zbq;
	}
	public String getZbr() {
		return zbr;
	}
	public void setZbr(String zbr) {
		this.zbr = zbr;
	}
	public String getZbs() {
		return zbs;
	}
	public void setZbs(String zbs) {
		this.zbs = zbs;
	}
	public String getZbt() {
		return zbt;
	}
	public void setZbt(String zbt) {
		this.zbt = zbt;
	}
	public String getZbu() {
		return zbu;
	}
	public void setZbu(String zbu) {
		this.zbu = zbu;
	}
	public String getZbv() {
		return zbv;
	}
	public void setZbv(String zbv) {
		this.zbv = zbv;
	}
	public String getZbw() {
		return zbw;
	}
	public void setZbw(String zbw) {
		this.zbw = zbw;
	}
	public String getZbx() {
		return zbx;
	}
	public void setZbx(String zbx) {
		this.zbx = zbx;
	}
	public String getZby() {
		return zby;
	}
	public void setZby(String zby) {
		this.zby = zby;
	}
	public String getZbz() {
		return zbz;
	}
	public void setZbz(String zbz) {
		this.zbz = zbz;
	}
	public String getZca() {
		return zca;
	}
	public void setZca(String zca) {
		this.zca = zca;
	}
	public String getZcb() {
		return zcb;
	}
	public void setZcb(String zcb) {
		this.zcb = zcb;
	}
	public String getZcc() {
		return zcc;
	}
	public void setZcc(String zcc) {
		this.zcc = zcc;
	}
	public String getZcd() {
		return zcd;
	}
	public void setZcd(String zcd) {
		this.zcd = zcd;
	}
	public String getZce() {
		return zce;
	}
	public void setZce(String zce) {
		this.zce = zce;
	}
	public String getZcf() {
		return zcf;
	}
	public void setZcf(String zcf) {
		this.zcf = zcf;
	}
	public String getZcg() {
		return zcg;
	}
	public void setZcg(String zcg) {
		this.zcg = zcg;
	}
	public String getZch() {
		return zch;
	}
	public void setZch(String zch) {
		this.zch = zch;
	}
	public String getZci() {
		return zci;
	}
	public void setZci(String zci) {
		this.zci = zci;
	}
	public String getZcj() {
		return zcj;
	}
	public void setZcj(String zcj) {
		this.zcj = zcj;
	}
	public String getZck() {
		return zck;
	}
	public void setZck(String zck) {
		this.zck = zck;
	}
	public String getZcl() {
		return zcl;
	}
	public void setZcl(String zcl) {
		this.zcl = zcl;
	}
	public String getZcm() {
		return zcm;
	}
	public void setZcm(String zcm) {
		this.zcm = zcm;
	}
	public String getZcn() {
		return zcn;
	}
	public void setZcn(String zcn) {
		this.zcn = zcn;
	}
	public String getZco() {
		return zco;
	}
	public void setZco(String zco) {
		this.zco = zco;
	}
	public String getZcp() {
		return zcp;
	}
	public void setZcp(String zcp) {
		this.zcp = zcp;
	}
	public String getZcq() {
		return zcq;
	}
	public void setZcq(String zcq) {
		this.zcq = zcq;
	}
	public String getZcr() {
		return zcr;
	}
	public void setZcr(String zcr) {
		this.zcr = zcr;
	}
	public String getZcs() {
		return zcs;
	}
	public void setZcs(String zcs) {
		this.zcs = zcs;
	}
	public String getZct() {
		return zct;
	}
	public void setZct(String zct) {
		this.zct = zct;
	}
	public String getZcu() {
		return zcu;
	}
	public void setZcu(String zcu) {
		this.zcu = zcu;
	}
	public String getZcv() {
		return zcv;
	}
	public void setZcv(String zcv) {
		this.zcv = zcv;
	}
	public String getZcw() {
		return zcw;
	}
	public void setZcw(String zcw) {
		this.zcw = zcw;
	}
	public String getZcx() {
		return zcx;
	}
	public void setZcx(String zcx) {
		this.zcx = zcx;
	}
	public String getZcy() {
		return zcy;
	}
	public void setZcy(String zcy) {
		this.zcy = zcy;
	}
	public String getZcz() {
		return zcz;
	}
	public void setZcz(String zcz) {
		this.zcz = zcz;
	}
	public String getZda() {
		return zda;
	}
	public void setZda(String zda) {
		this.zda = zda;
	}
	public String getZdb() {
		return zdb;
	}
	public void setZdb(String zdb) {
		this.zdb = zdb;
	}
	public String getZdc() {
		return zdc;
	}
	public void setZdc(String zdc) {
		this.zdc = zdc;
	}
	public String getZdd() {
		return zdd;
	}
	public void setZdd(String zdd) {
		this.zdd = zdd;
	}
	public String getZde() {
		return zde;
	}
	public void setZde(String zde) {
		this.zde = zde;
	}
	public String getZdf() {
		return zdf;
	}
	public void setZdf(String zdf) {
		this.zdf = zdf;
	}
	public String getZdg() {
		return zdg;
	}
	public void setZdg(String zdg) {
		this.zdg = zdg;
	}
	public String getZdh() {
		return zdh;
	}
	public void setZdh(String zdh) {
		this.zdh = zdh;
	}
	public String getZdi() {
		return zdi;
	}
	public void setZdi(String zdi) {
		this.zdi = zdi;
	}
	public String getZdj() {
		return zdj;
	}
	public void setZdj(String zdj) {
		this.zdj = zdj;
	}
	public String getZdk() {
		return zdk;
	}
	public void setZdk(String zdk) {
		this.zdk = zdk;
	}
	public String getZdl() {
		return zdl;
	}
	public void setZdl(String zdl) {
		this.zdl = zdl;
	}
	public String getZdm() {
		return zdm;
	}
	public void setZdm(String zdm) {
		this.zdm = zdm;
	}
	public String getZdn() {
		return zdn;
	}
	public void setZdn(String zdn) {
		this.zdn = zdn;
	}
	public String getZdo() {
		return zdo;
	}
	public void setZdo(String zdo) {
		this.zdo = zdo;
	}
	public String getZdp() {
		return zdp;
	}
	public void setZdp(String zdp) {
		this.zdp = zdp;
	}
	public String getZdq() {
		return zdq;
	}
	public void setZdq(String zdq) {
		this.zdq = zdq;
	}
	public String getZdr() {
		return zdr;
	}
	public void setZdr(String zdr) {
		this.zdr = zdr;
	}
	public String getZds() {
		return zds;
	}
	public void setZds(String zds) {
		this.zds = zds;
	}
	public String getZdt() {
		return zdt;
	}
	public void setZdt(String zdt) {
		this.zdt = zdt;
	}
	public String getZdu() {
		return zdu;
	}
	public void setZdu(String zdu) {
		this.zdu = zdu;
	}
	public String getZdv() {
		return zdv;
	}
	public void setZdv(String zdv) {
		this.zdv = zdv;
	}
	public String getZdw() {
		return zdw;
	}
	public void setZdw(String zdw) {
		this.zdw = zdw;
	}
	public String getZdx() {
		return zdx;
	}
	public void setZdx(String zdx) {
		this.zdx = zdx;
	}
	public String getZdy() {
		return zdy;
	}
	public void setZdy(String zdy) {
		this.zdy = zdy;
	}
	public String getZdz() {
		return zdz;
	}
	public void setZdz(String zdz) {
		this.zdz = zdz;
	}
	public String getZea() {
		return zea;
	}
	public void setZea(String zea) {
		this.zea = zea;
	}
	public String getZeb() {
		return zeb;
	}
	public void setZeb(String zeb) {
		this.zeb = zeb;
	}
	public String getZec() {
		return zec;
	}
	public void setZec(String zec) {
		this.zec = zec;
	}
	public String getZed() {
		return zed;
	}
	public void setZed(String zed) {
		this.zed = zed;
	}
	public String getZee() {
		return zee;
	}
	public void setZee(String zee) {
		this.zee = zee;
	}
	public String getZef() {
		return zef;
	}
	public void setZef(String zef) {
		this.zef = zef;
	}
	public String getZeg() {
		return zeg;
	}
	public void setZeg(String zeg) {
		this.zeg = zeg;
	}
	public String getZeh() {
		return zeh;
	}
	public void setZeh(String zeh) {
		this.zeh = zeh;
	}
	public String getZei() {
		return zei;
	}
	public void setZei(String zei) {
		this.zei = zei;
	}
	public String getZej() {
		return zej;
	}
	public void setZej(String zej) {
		this.zej = zej;
	}
	public String getZek() {
		return zek;
	}
	public void setZek(String zek) {
		this.zek = zek;
	}
	public String getZel() {
		return zel;
	}
	public void setZel(String zel) {
		this.zel = zel;
	}
	public String getZem() {
		return zem;
	}
	public void setZem(String zem) {
		this.zem = zem;
	}
	public String getZen() {
		return zen;
	}
	public void setZen(String zen) {
		this.zen = zen;
	}
	public String getZeo() {
		return zeo;
	}
	public void setZeo(String zeo) {
		this.zeo = zeo;
	}
/*	@Override
	public String toString() {
		return "NewData [id=" + id + ", zaa=" + zaa + ", zab=" + zab + ", zac=" + zac + ", zad=" + zad + ", zae=" + zae
				+ ", zaf=" + zaf + ", zag=" + zag + ", zah=" + zah + ", zai=" + zai + ", zaj=" + zaj + ", zak=" + zak
				+ ", zal=" + zal + ", zam=" + zam + ", zan=" + zan + ", zao=" + zao + ", zap=" + zap + ", zaq=" + zaq
				+ ", zar=" + zar + ", zas=" + zas + ", zat=" + zat + ", zau=" + zau + ", zav=" + zav + ", zaw=" + zaw
				+ ", zax=" + zax + ", zay=" + zay + ", zaz=" + zaz + ", zba=" + zba + ", zbb=" + zbb + ", zbc=" + zbc
				+ ", zbd=" + zbd + ", zbe=" + zbe + ", zbf=" + zbf + ", zbg=" + zbg + ", zbh=" + zbh + ", zbi=" + zbi
				+ ", zbj=" + zbj + ", zbk=" + zbk + ", zbl=" + zbl + ", zbm=" + zbm + ", zbn=" + zbn + ", zbo=" + zbo
				+ ", zbp=" + zbp + ", zbq=" + zbq + ", zbr=" + zbr + ", zbs=" + zbs + ", zbt=" + zbt + ", zbu=" + zbu
				+ ", zbv=" + zbv + ", zbw=" + zbw + ", zbx=" + zbx + ", zby=" + zby + ", zbz=" + zbz + ", zca=" + zca
				+ ", zcb=" + zcb + ", zcc=" + zcc + ", zcd=" + zcd + ", zce=" + zce + ", zcf=" + zcf + ", zcg=" + zcg
				+ ", zch=" + zch + ", zci=" + zci + ", zcj=" + zcj + ", zck=" + zck + ", zcl=" + zcl + ", zcm=" + zcm
				+ ", zcn=" + zcn + ", zco=" + zco + ", zcp=" + zcp + ", zcq=" + zcq + ", zcr=" + zcr + ", zcs=" + zcs
				+ ", zct=" + zct + ", zcu=" + zcu + ", zcv=" + zcv + ", zcw=" + zcw + ", zcx=" + zcx + ", zcy=" + zcy
				+ ", zcz=" + zcz + ", zda=" + zda + ", zdb=" + zdb + ", zdc=" + zdc + ", zdd=" + zdd + ", zde=" + zde
				+ ", zdf=" + zdf + ", zdg=" + zdg + ", zdh=" + zdh + ", zdi=" + zdi + ", zdj=" + zdj + ", zdk=" + zdk
				+ ", zdl=" + zdl + ", zdm=" + zdm + ", zdn=" + zdn + ", zdo=" + zdo + ", zdp=" + zdp + ", zdq=" + zdq
				+ ", zdr=" + zdr + ", zds=" + zds + ", zdt=" + zdt + ", zdu=" + zdu + ", zdv=" + zdv + ", zdw=" + zdw
				+ ", zdx=" + zdx + ", zdy=" + zdy + ", zdz=" + zdz + ", zea=" + zea + ", zeb=" + zeb + ", zec=" + zec
				+ ", zed=" + zed + ", zee=" + zee + ", zef=" + zef + ", zeg=" + zeg + ", zeh=" + zeh + ", zei=" + zei
				+ ", zej=" + zej + ", zek=" + zek + ", zel=" + zel + ", zem=" + zem + ", zen=" + zen + ", zeo=" + zeo
				+ "]";
	}*/
	
	
	
	private String adviseloan;
	private String adviseratio;
	public String getAdviseloan() {
		return adviseloan;
	}
	public void setAdviseloan(String adviseloan) {
		this.adviseloan = adviseloan;
	}
	public String getAdviseratio() {
		return adviseratio;
	}
	public void setAdviseratio(String adviseratio) {
		this.adviseratio = adviseratio;
	}
	
	private String creditgrade;
	private String badratio;
	public String getCreditgrade() {
		return creditgrade;
	}
	public void setCreditgrade(String creditgrade) {
		this.creditgrade = creditgrade;
	}
	public String getBadratio() {
		return badratio;
	}
	public void setBadratio(String badratio) {
		this.badratio = badratio;
	}
	@Override
	public String toString() {
		return  id + "|" + zaa + "|" + zab + "|" + zac + "|" + zad + "|" + zae
				+ "|" + zaf + "|" + zag + "|" + zah + "|" + zai + "|" + zaj + "|" + zak
				+ "|" + zal + "|" + zam + "|" + zan + "|" + zao + "|" + zap + "|" + zaq
				+ "|" + zar + "|" + zas + "|" + zat + "|" + zau + "|" + zav + "|" + zaw
				+ "|" + zax + "|" + zay + "|" + zaz + "|" + zba + "|" + zbb + "|" + zbc
				+ "|" + zbd + "|" + zbe + "|" + zbf + "|" + zbg + "|" + zbh + "|" + zbi
				+ "|" + zbj + "|" + zbk + "|" + zbl + "|" + zbm + "|" + zbn + "|" + zbo
				+ "|" + zbp + "|" + zbq + "|" + zbr + "|" + zbs + "|" + zbt + "|" + zbu
				+ "|" + zbv + "|" + zbw + "|" + zbx + "|" + zby + "|" + zbz + "|" + zca
				+ "|" + zcb + "|" + zcc + "|" + zcd + "|" + zce + "|" + zcf + "|" + zcg
				+ "|" + zch + "|" + zci + "|" + zcj + "|" + zck + "|" + zcl + "|" + zcm
				+ "|" + zcn + "|" + zco + "|" + zcp + "|" + zcq + "|" + zcr + "|" + zcs
				+ "|" + zct + "|" + zcu + "|" + zcv + "|" + zcw + "|" + zcx + "|" + zcy
				+ "|" + zcz + "|" + zda + "|" + zdb + "|" + zdc + "|" + zdd + "|" + zde
				+ "|" + zdf + "|" + zdg + "|" + zdh + "|" + zdi + "|" + zdj + "|" + zdk
				+ "|" + zdl + "|" + zdm + "|" + zdn + "|" + zdo + "|" + zdp + "|" + zdq
				+ "|" + zdr + "|" + zds + "|" + zdt + "|" + zdu + "|" + zdv + "|" + zdw
				+ "|" + zdx + "|" + zdy + "|" + zdz + "|" + zea + "|" + zeb + "|" + zec
				+ "|" + zed + "|" + zee + "|" + zef + "|" + zeg + "|" + zeh + "|" + zei
				+ "|" + zej + "|" + zek + "|" + zel + "|" + zem + "|" + zen + "|" + zeo
				+ "|" + adviseloan + "|" + adviseratio + "|" + creditgrade
				+ "|" + badratio ;
	}
	
	
	
	
	
	
}
